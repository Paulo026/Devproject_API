...
public class UsuarioService {
	...
	public UsuarioDTO buscar(Integer idUsuario) {
		return uRepository.findById(idUsuario).map(usuario -> mapToDTO(usuario, new UsuarioDTO()))
	...
	public List<UsuarioDTO> buscarTodos() {
		return uRepository.findAll().stream().map(usuario -> mapToDTO(usuario, new UsuarioDTO()))
	...
	public UsuarioDTO buscarPorLogin(String login) {
		return uRepository.findAll().stream().filter(usuario -> usuario.getUsername().equals(login))
				.map(usuario -> mapToDTO(usuario, new UsuarioDTO())).findFirst()
				.orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND));
	}
